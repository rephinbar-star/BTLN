  ...
  return (
    <div className="space-y-4">
      ...
      {preview && preview.format !== "screenshots_pending" && <section className="rounded-lg border border-border bg-muted/30 p-4" aria-label="Conversation preview">
        ...
        {value.method === "screenshots" && (editingPreview ? <div className="mt-3"><label htmlFor={`screenshot-correction-${preview.id}`} className="text-sm font-semibold">Correct speakers or order</label><textarea id={`screenshot-correction-${preview.id}`} value={previewText} onChange={(event) => setPreviewText(event.target.value)} rows={7} className="mt-1 w-full rounded-md border border-input bg-background p-3 text-sm leading-relaxed"/><div className="mt-2 flex flex-wrap gap-2"><Button type="button" className="min-h-11" onClick={() => { try { const parsed = parseTranscript(previewText); const corrected = canonicalizeParsedConversation(parsed, "screenshots", preview.sourceName); corrected.warnings = [...preview.warnings, "Speaker or order corrections were supplied by the uploader."]; patch({ conversation: corrected, selfParticipantId: corrected.participants.find((person) => person.display_name === "You")?.id ?? null }); setEditingPreview(false); setError(null); } catch (cause) { setError(cause instanceof Error ? cause.message : "We couldn't read those corrections."); } }}>Save corrections</Button><Button type="button" variant="ghost" className="min-h-11" onClick={() => setEditingPreview(false)}>Cancel</Button></div></div> : <Button type="button" variant="outline" className="mt-3 min-h-11" onClick={() => { setPreviewText(preview.messages.map((message) => `${message.raw_sender ?? "Unknown"}: ${message.content}`).join("\n")); setEditingPreview(true); }}>Correct speakers or order</Button>)}
