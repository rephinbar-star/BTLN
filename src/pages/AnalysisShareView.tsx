...
import { hashShareToken, type GroupShareSnapshot } from "@/lib/group/types";

const PENDING_KEY = "btln_analysis_perspective_pending";

/**
 * Recipient view for a two-person (Deep Read) share link.
 *
 * Everything on this page comes from the snapshot the owner deliberately
 * released. The private report is never reachable from here, and picking a
 * role is self-reported — it is not identity verification and grants nothing.
 */
const AnalysisShareView = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const { data } = await supabase.rpc("resolve_analysis_share", { p_token_hash: hash });
        if (cancelled) return;
        if (!data) {
          setState("gone");
          return;
  ...
  const choose = (i: number) => {
    ...
    track("analysis_perspective_selected", {});
  };
  ...
  const savePerspective = useCallback(async () => {
    ...
    setSaveError(null);
    const hash = await hashShareToken(token);
    const { data, error } = await supabase.rpc("save_analysis_recipient_perspective", {
      p_token_hash: hash,
    ...
    track("analysis_perspective_saved", {});
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <Helmet>
        <title>A shared read | BetweenTheLines</title>
        <meta name="description" content="Someone shared how their conversation reads." />
      ...
      <main className="mx-auto max-w-2xl px-5 pb-20 pt-12 sm:px-8">
        ...
        {state === "gone" && (
          <div className="rounded-2xl border border-border bg-card p-6">
            ...
            <Link
              to="/"
              onClick={() => track("analysis_share_conversion", {})}
        ...
        {state === "ok" && snapshot && (
          <>
            <p className="text-sm text-muted-foreground">Deep Read · two people</p>
            ...
            {card && (
              <div className="mt-6 rounded-2xl border border-border bg-muted/30 p-6">
                ...
                {card.why && <p className="mt-2 text-[15px] text-muted-foreground">{card.why}</p>}
                {snapshot.strengths.length > 0 && (
                  <p className="mt-4 text-[15px]">
                    <span className="font-medium">One strength here: </span>
                ...
                <div className="mt-5">
                  {saved ? (
                    ...
                    <>
                      ...
                      {saveError && <p className="mt-2 text-[14px] text-destructive">{saveError}</p>}
                    </>
                  ) : (
                    <Link
            ...
            <p className="mt-6 text-center text-[13px] text-muted-foreground">
              No conversation text is shared on this page.
            </p>
          </>
        )}
      </main>
...
export default AnalysisShareView;
