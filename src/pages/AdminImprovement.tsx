      ...
      {job && (
        <Section title={`Comparison — ${jobMode?.label ?? job.mode} · ${versionById.get(job.candidate_id)?.label ?? ""}`}>
          <div className="rounded-lg border border-btln-line p-3 text-[13px]">
            ...
            <p className="mt-1 text-muted-foreground break-all">Job {job.id} · binding {short(job.binding_hash)} · baseline {short(versionById.get(job.baseline_id)?.content_hash)} · candidate {short(versionById.get(job.candidate_id)?.content_hash)} · parity {typeof job.parity === "object" ? String(job.parity?.parity ?? "unknown") : job.parity}</p>
