...
Deno.serve(async(req)=>{
 ...
 if(action==="resolve"){
   ...
   const tokenHash=await hash(token); const {data:link}=await db.from("group_roast_share_links").select("id,snapshot_json").eq("token_hash",tokenHash).is("revoked_at",null).maybeSingle();
